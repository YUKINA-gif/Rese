      ...
      <div class="wrap store_flex" v-else>
        <div class="flex store_card" v-for="(store, index) in stores" :key="index">
          ...
          <div>
            ...
            <button class="button store_button delete_button" @click="openModalDel(store)">
              店舗情報削除
            </button>
          </div>        
        </div>
      <!-- 店舗情報更新画面 -->
        <Modal v-if="modal" @close="closeModal" :val="sendStoreData" :areaData="areaData" :genreData="genreData"></Modal>
        <!-- 店舗削除画面 -->
        <ModalDel
          v-if="modal_del"
          @close="closeModalDel"
